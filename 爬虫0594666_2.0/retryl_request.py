...
def _parse_url(url, headers, proxies_list, method):
    if method == "GET":
        response = requests.get(url, headers=headers, proxies=random.choices(proxies_list)[0], verify=False, timeout=5)
    else:
...
def request_url(url, headers, proxies_list, method="GET"):
    try:
        html = _parse_url(url, headers, proxies_list, method)
    except:
        html = None
    return html
