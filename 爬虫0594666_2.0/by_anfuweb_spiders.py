...
class By_Anfuweb_Spiders():
    ...
    def nav_url_spider(self):
        """
        获取导航栏的各大分类连接保存数据库，然后读取信息封装成列表返回
        :return: [id, kind_href] 分类id，分类连接
        """
    ...
    def store_url_spider(self, kind_id, kind_url, temp_list=None):
        """
        利用递归函数进行翻页获取到一个分类里面的所有商铺，保存到temp_list中传递到get_store_info方法中
        :param kind_id: 分类id
        :param kind_url: 分类连接
        :param temp_list: 临时存放数据的列表
        :return: get_store_info 获取商店信息的方法
        """
    ...
    def get_store_info(self, store_href_list, kind_id):
        """
        获取所有商铺的信息，构造插入语句，放进队列中
        :param store_href_list: 一个分类里面的所有商铺连接
        :param kind_id: 分类ID
        :return: None
        """
    ...
    def run(self):
        ...
        for t in thread_list:
            t.start()
        # By_Img_Spiders(self.ip_pool).run()
