...
def begin_install(comm_list):
    os.system(comm_list)
    return "Installation completed, please check !"
...
def implement_install(install_list, ip_str, mac_str):
    ...
    while True:
        if i >= len(str_install):
            break
        # print(i)
        temp_str = " autosetup.sh -i " + str_install[i] + " -c " + str_check[i] + " -h " + ip_str + " -n " + mac_str # + " >/dev/null 2>&1 "
        # print(temp_str)
        comm_list.append(temp_str)
        i += 1
    # print(comm_list)
    pool = Pool(len(str_install))
    install_info = pool.map(begin_install, comm_list)
    pool.close()
    pool.join()
    print(install_info)
