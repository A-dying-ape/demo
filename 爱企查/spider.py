...
def get_detail(pid, e):
    ...
    try:
        j_str = json.loads(response.content.decode("unicode_escape"))
    except:
        print(response.url + "出现错误,可能是网络原因！")
    basicData = j_str.get("data").get("basicData")
    entName = str(basicData.get("entName"))
    regNo = str(basicData.get("regNo"))
    e.append(regNo)
    e.append(entName)
    if e[0] == e[5]:
        e.append("完全匹配")
...
def get_pid(e):
    ...
    response = requests.get(url, headers=headers, params=params, proxies=random.choice(PROXY))
    res = re.findall("\"resultList\":\[\{\"pid\":\"(.*?)\",\"entName\":\"", response.content.decode("unicode_escape"))
    if len(res) <= 0:
        e.append("")
        e.append("")
        e.append("")
...
def get_excel_msg():
    ...
    sheet_name = ""
    path = "社会代码核查表.xlsx"
    e = excel.MakeExcel(xls_name, sheet_name, path)
    print("开始处理数据，耐心等待......")
    for i in range(3):
        res = e.read_sheet(i)
        for j in res:
            if j is not None and len(j[0]) > 3:
                q.put(j[0:3])
...
if __name__ == '__main__':
    t_list = []
    title = ["户名", "地址", "账务联系人", "工商行政名称", "统一社会信用代码", "爬取名称", "备注"]
    final_list.append(title)
    get_excel_msg()
    for i in range(10):
        t_list.append(threading.Thread(target=run))
    for t in t_list:
        t.start()
        t.join()
    print("数据处理完成,开始保存数据！")
    handle_data(final_list)
    print("已完成所有处理,最终数据保存在dataframe.csv文件中！")
